...
const JobDescription = () => {
    ...
    return (<div className="w-full flex flex-col gap-8">
      <div className="w-full flex items-start gap-5 bg-white rounded-lg p-9 shadow-sm border border-gray-200 flex-col">
        ...
        <p 
        className="text-gray-600 leading-7" dangerouslySetInnerHTML={{__html: job.description || "" }}
        ></p>
      ...
      <div className="w-full flex flex-col gap-4 items-center">
        <div className="w-full text-indigo-500 font-semibold text-xl text-center bg-white rounded-lg p-3 mb-4 border border-gray-200">Similar Jobs
        </div>
                  ...
                  <div className="flex flex-col items-start gap-3">
                    <img src={job.logo} alt={job.title} className="w-[70px]"
                     ...
                     <div className="flex items-center gap-2">
                      <BusinessOutline width={"18px"} height={"18px"} color="#5555"/>
                      <span className="text-[14px] font-medium text-gray-600">{job.company}
                      </span>
                     </div>
                     <div className="flex items-center gap-2">
                      <LocationOutline width={"18px"} height={"18px"} color="#5555"/>
                      <span className="text-[14px] font-medium text-gray-600">{job.workStatus}
                      </span>
                     </div>
                     <div className="flex items-center gap-2">
                      <NewspaperOutline width={"18px"} height={"18px"} color="#5555"/>
                      <span className="text-[14px] font-medium text-gray-600">{job.contractStatus}
                      </span>
                     </div>
                  </div>
                  <div className="flex flex-col gap-4 sel-end">
                    <button onClick={() => navigate(`/jobs/${job.id}`)}
                    className="text-white font-bold text-lg rounded-md bg-indigo-500 w-10 h-10"
                    >
                      Apply
                    </button>
                  </div>
