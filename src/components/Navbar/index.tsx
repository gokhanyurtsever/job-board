...
const Navbar = () => {
  ...
  return <div className="w-full h-[60px] bg-white shadow-sm absolute top-0 flex z-10"> 
...
<div className="hidden md:flex items-center gap-6">
    {navLinks.map((navlink) => {
        return(
            <a href={navlink.path} key={navlink.title} className={`font-medium text-[15px] ${navlink.active ? "text-indiho-500" : "text-[#828ea8]"} transition-all duration-200 hover:text-indigo-500`}
            >
                {navlink.title}
